...
try:
    ...
    print('please run: pip install hiq-python>=1.1.9')
    exit()

"""
requirements:
    pip install hiq-python>=1.1.9


Sample usage:

    ```
    python convert_llama.py --to [hf|fb] --ckpt_dir $CKPT_DIR --tokenizer_path $TOKENIZER_PATH --model 7B --output_dir $CKPT_DIR --max_batch_size 4
    ```

    hf models can be loaded via:
    ```
...
def get_args():
    ...
    parser.add_argument("--model_size",choices=NUM_SHARDS.keys(),)
